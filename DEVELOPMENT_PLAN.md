...
* [ ] Убедиться, что shadcn/ui уже инициализирован в проекте.
* [ ] Добавить компоненты:
...
* [ ] Импортировать `Toaster` из `@/components/ui/sonner`.
* [ ] Вставить `<Toaster />` в `body` (в конце), один раз.

Контроль:

* [ ] Ни в одном месте не использовать `alert`.
* [ ] Ошибки/успех показывать через `toast(...)` из `sonner`.
...
* [ ] `lib/random/prng.ts`
* [ ] `lib/generators/task9/types.ts`
* [ ] `lib/generators/task9/text.ts`
* [ ] `lib/generators/task9/area.ts`
* [ ] `lib/generators/task9/spokes.ts`
* [ ] `lib/generators/task9/fence.ts`
* [ ] `lib/generators/task9/validate.ts`
* [ ] `lib/generators/task9/index.ts`
...
* [ ] Реализовать `createPrng(seed: number)`:
...
* [ ] При одинаковом `seed` и одинаковых настройках набор задач повторяется.
...
* [ ] `answer` в задачах — только число, без единиц.
...
* [ ] генерирует параметры в зависимости от `difficulty`
* [ ] вычисляет `answer` (строкой)
* [ ] берет `statement/steps` из `text.ts`
* [ ] заполняет `params` исходными числами
...
* [ ] Реализовать `validateTask(task: Task): void` с проверками:
...
* [ ] В процессе генерации одной задачи использовать `MAX_ATTEMPTS = 50`:
...
* [ ] Реализовать `generateTask9Set(options: GenerateOptions): Task[]`

Внутри:

* [ ] Нормализовать `count`: целое, зажать 1..50.
* [ ] Нормализовать `seed`: если NaN/пусто — считать “не задан”.
* [ ] Создать `prng` на основе seed (или `Date.now()`).
* [ ] Генерировать `count` задач:
...
* [ ] Все сообщения только через `toast(...)` из `sonner`.

### Состояния

* [ ] `count`, `subtype`, `difficulty`, `seed`
* [ ] `tasks: Task[]`
* [ ] `sheetOpen` (для мобильного режима)

### Валидация ввода на UI

* [ ] Если `count` некорректен → `toast("Некорректное количество задач")` и остановить.
* [ ] Если `seed` заполнен, но некорректен → `toast("Некорректное начальное число")` и остановить **или** игнорировать seed (выбрать один вариант и придерживаться везде). Рекомендация: остановить и попросить исправить.
...
* [ ] `grid` с двумя колонками:
  ...
  * справа список задач
* [ ] Кнопка “Сгенерировать” в левом блоке
* [ ] `toast("Сгенерировано: N")` при успехе
...
* [ ] (Опционально) `Button` “Скопировать ответ”:
...
* [ ] Обернуть генерацию в `try/catch`:
...
* [ ] Добавить утилиту `formatNumberRu` и комментарий: если появятся дроби, использовать запятую (`0,6`).

---

# 5) Финальная проверка (приёмка)

* [ ] Генерируется ровно N задач.
* [ ] Выбранный тип генерирует только этот тип, “Смешанный” даёт смесь.
* [ ] Одинаковый seed даёт одинаковый набор при тех же настройках.
* [ ] `answer` везде только число, без единиц.
* [ ] Решение совпадает с вычислениями из `params`.
* [ ] На телефоне/планшете настройки в Sheet, на компьютере — слева в колонке.
* [ ] Все ошибки/уведомления идут только через Sonner.
